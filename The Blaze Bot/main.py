...
async def on_ready():
    await client.change_presence(
        activity=discord.Game('Basketball Gm'))
...
prefix = '+'
nonExportCommands = ['help', 'load', 'test', 'team-list', 'stuff']
            ...
            if command == 'load':
                if (message.author.guild_permissions.manage_roles or message.author.id in adminUsers):
                    ...
                    if exportCheck == True:
                        ...
                        await message.channel.send("Successfully Loaded!")
                    if exportCheck == False:
                        await message.channel.send("Sorry that doesn't look like a valid URL.")
                else:
                    await message.channel.send("You do not have permission to load the exports")
        if command in totalCommands:
            print(message.author.name, 'in', message.guild.name, ': ', message.content)
            ...
            if command == 'stuff':
                ...
                if x == 1:
                    await message.channel.send("Michael Jordan is still the #1 Goat🐐")
                if x == 2:
                    await message.channel.send("Stephen Curry is the best PG of all time not Magic😡")
                if x == 3:
                    await message.channel.send("The Golden State Warriors will deliver Chris Paul his missing ring😎")
                if x == 4:
                    await message.channel.send("Karl Malone finishes Dunks, not court accusations - **Santoss 2023**")
                if x == 5:
                    await message.channel.send("Miles Bridges makes dunks, not assaults - **Santoss 2023**")
          

            if command == 'help':
                embed = discord.Embed(
                        title='**__BlazeBot Command List__**',
                        description=f'Currently assisting {len(client.guilds)} servers, The BlazeBot pulls information from Basketball GM exports t5eo you!',
                        color=0xE7AA00)
                embed.set_footer(text="This Version is created by MasterJ#3117")
              
                if message.channel.guild.me.guild_permissions.manage_messages == False:
                    embed.add_field(name='Permission needed', value='The BlazeBot requires "manage messages" permissions in order to properly switch between embed screens on commands such as roster and stats.')
...
• **+proghistory**
• **+awards**
• **+compare** (compares a players ceiling to a player)
                """, inline=False)
                embed.add_field(name="**Team Commands**",value="""
• **+team-list** (Shows a list teams that have current gms and shows which teams are available *Coming Soon*)
• **+roster** (shows both contracts and stats of the player)
• **+picks** (shows the capital of draft picks a team owns)
• **+ownspicks** (shows who owns which draft picks of said team)
• **+sos** (shows SOS based on win percentage of opposing teams)
• **+pyramid** (shows projected record based on TR, and SOS)
                """, inline=False)
                embed.add_field(name="**League Commands**",value="""
• **+fa(shows a list of available free agents**)
• **+draft** (shows draft ratings historically)
                """, inline=True)
                embed.add_field(name="**Other**",value="""
• **+load** (server mods are the only ones who have permission to use this, loads an export file)
• **+help** (Brings you a list of commands that this bot has)
        ...
        if command in playerCommands:
                        ...
                        async def find_stats(playoff):
                            ...
                            if totalGames == 0:
                                ...
                                if playoff == True:
                                    ...
                                    secondaryEmbed.set_footer(text="Click the ⬅️  arrow to see players regular season stats | This Version is created by MasterJ#3117")
                            ...
                            async def find_stats(playoff):
                                ...
                                if totalGames == 0:
                                    ...
                                    if playoff == True:
                                        ...
                                        secondaryEmbed.add_field(name='Efficiency',value=effStatline, inline=False)
                                        secondaryEmbed.add_field(name=bottomHeader,value=bottomText, inline=False)
                                        secondaryEmbed.set_footer(text="Click the ⬅️  arrow to see plsyers regular season stats | This Version is created by MasterJ#3117")
                                        secondaryEmbed.set_thumbnail(url=image)
                                else:
                                    averageMins = round(totalMins / totalGames, 1)
                                    averagePer = round(totalPer / totalGames, 1)
            ...
            if command == 'compare':
                if int(playerAge) > 4:
                    regularEmbed.add_field(name='Error', value='Sorry! We dont compare retired players')
                else:
                    for p in players:
                        if p['pid'] == winningPid:
                            if sport == 'basketball':
            ...
            if secondEmbed == True:
                regularEmbed.set_footer(text="Click the ➡️  arrow to see players playoff stats | This Version is created by MasterJ#3117")
            else:
                regularEmbed.set_footer(text="This Version is created by MasterJ#3117")
            ...
            if secondEmbed == True:
                ...
                for i in range(10):
                    ...
                    try:
                        ...
                        except: await message.channel.send('>>> ---' + '\n' + ' **__Permission Needed__:** BlazeBot requires "manage messages" permissions in order to properly switch back-and-forth between embed screens.' + '\n' + '---')
                    def check(reaction, user):
                        return reaction.message == botEmbed and user == message.author and str(reaction.emoji) == '⬅️'
                    try:
                        reaction, user = await client.wait_for('reaction_add', timeout=120.0, check=check)
        ...
        if command in teamCommands:
            ...
            if command == 'roster':
                ...
                if commandSeason == "":
                    ...
                    if rosterOverflow:
                        regularEmbed.add_field(name="A quick heads up ⬇️", value="Sadly, not every player are in this list simply because they just dont fit in due to Discord's character limit")
                        secondaryEmbed.add_field(name="A quick heads up ⬇️", value="Sadly, not every player are in this list simply because they just dont fit in due to Discord's character limit")
                    ...
                    secondaryEmbed.set_footer(text="Click the ⬅️  arrow to see players contracts | This Version is created by MasterJ#3117")   
                    secondEmbed = True
                else:
            ...
            if command == 'pyramid':
                ...
                secondaryEmbed.set_footer(text="Click the ⬅️  arrow to go pack to the pyramid wins projection | This Version is created by MasterJ#3117")   
                secondEmbed = True
            ...
            if secondEmbed == True:
                if command == 'roster':
                    regularEmbed.set_footer(text="Click the ➡️  arrow to see players stats | This Version is created by MasterJ#3117")
                if command == 'pyramid':
                    regularEmbed.set_footer(text='Click the ➡️  arrow to see how pyramid wins work | This Version is created by MasterJ#3117')
            else:
                regularEmbed.set_footer(text="This Version is created by MasterJ#3117")
        ...
        if command in leagueCommands:
            ...
            if command == 'fa':
                ...
                secondaryEmbed.set_footer(text='Click the ⬅️  arrow to sort by OVR | This Version is created by MasterJ#3117')
                secondEmbed = True
            ...
            if command == 'deaths':
                ...
                if secondEmbed == True:
                    ...
                    secondaryEmbed.set_footer(text='This Version is created by MasterJ#3117')
                    secondEmbed = True
            ...
            if secondEmbed == True:
                if command == 'fa':
                    regularEmbed.set_footer(text="Click the ➡️  arrow to sort by POT | This Version is created by MasterJ#3117")
